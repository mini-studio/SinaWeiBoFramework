...
- (void)releaseRequestBlock
{
    if ( weiBoRequestBlock )
    {
        Block_release( weiBoRequestBlock );
        weiBoRequestBlock = nil;
    }
}

- (void)dealloc
{
    [self releaseRequestBlock];
...
- (void)logIn
{
    if ([self isLoggedIn])
    {
        ...
        if ( weiBoRequestBlock )
        {
            weiBoRequestBlock ( [NSError errorWithDomain:@"SinaWeiBo" code:KError_AlreadyLoggedIn userInfo:nil],nil);
        }
        if (isUserExclusive)
        {
            return;
...
- (void)setRequestBlock:(void (^)(NSError *error,id data))block
{
    if ( weiBoRequestBlock )
    {
        Block_release( weiBoRequestBlock );
    ...
    if ( block )
    {
        weiBoRequestBlock = Block_copy( block );
    }
}
                 ...
                 httpHeaderFields:(NSDictionary *)httpHeaderFields
{
	...
	if (![self isLoggedIn])
	{
        ...
        if ( weiBoRequestBlock )
        {
            weiBoRequestBlock ([NSError errorWithDomain:@"SinaWeiBo" code:KError_NotAuthorized userInfo:nil],nil);
        }
        return;
	}
    ...
    if ([self isAuthorizeExpired])
    {
        ...
        if ( weiBoRequestBlock )
        {
            weiBoRequestBlock ([NSError errorWithDomain:@"SinaWeiBo" code:KError_AuthorizeExpired userInfo:nil],nil);
        }
...
- (void)authorize:(WBAuthorize *)authorize didFailWithError:(NSError *)error
{
    ...
    if ( weiBoRequestBlock )
    {
        weiBoRequestBlock(error,nil);
    }
}

- (void)authorize:(WBAuthorize *)authorize viewWillAppear:(UIView *)view
{
    if ([delegate respondsToSelector:@selector(engine:authViewAppear:)])
    {
        [delegate engine:self authViewAppear:view];
    }
}

- (void)authorizeWebViewRequestStart:(WBAuthorize *)authorize
{
    [delegate engineRequestStart:self];
}

- (void)authorizeWebViewRequestEnd:(WBAuthorize *)authorize error:(NSString*)error
{
    [delegate engineRequestEnd:self];
}
