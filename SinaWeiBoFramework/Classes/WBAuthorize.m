...
@property (nonatomic,retain)WBAuthorizeWebView *webView;
...
- (void)startAuthorize
{
    ...
    self.webView = [[[WBAuthorizeWebView alloc] initWithFrame:CGRectMake(0, 0, 320, 440)] autorelease];
    [self.webView setDelegate:self];
    [self.webView loadRequestWithURL:[NSURL URLWithString:urlString]];
    [self.delegate authorize:self viewWillAppear:webView];
    [self.webView show:NO];
}
...
- (void)authorizeWebView:(WBAuthorizeWebView *)aWebView didReceiveAuthorizeCode:(NSString *)code
{
    [aWebView hide:YES];
...
- (void)authorizeWebViewRequestStart
{
    [self.delegate authorizeWebViewRequestStart:self];
}

- (void)authorizeWebViewRequestEnd:(NSString*)error
{
    [self.delegate authorizeWebViewRequestEnd:self error:error];
}
